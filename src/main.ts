...
function renderRulesList() {
  ...
  if (rules.length === 0) {
    rulesList.innerHTML = '<div class="empty-rules" style="color: var(--text-muted); font-size: 0.8rem; text-align: center; padding: 1.5rem 0;">No custom rules defined yet. Add one!</div>';
    return;
  }
...
function runConversion() {
  ...
  for (const match of filteredFields) {
    ...
    for (const cand of rawCandidates) {
      ...
      const hintJournal = cand.matchType === 'abbreviated'
        ? (cand.originalKey ?? match.value)
        : match.value;
...
function updateUnificationSection() {
  ...
  issues.forEach(issue => {
    ...
    if (issue.type === 'missing') {
      ...
      const rawList = Array.from(new Set(issue.group.entries.map(e => `"${e.match.value}"`))).join(', ');
